...
const pendingTextBlockPersistences = new Set();

function trackTextBlockPersistence(persistence) {
  pendingTextBlockPersistences.add(persistence);
  persistence
    .finally(() => pendingTextBlockPersistences.delete(persistence))
    .catch(() => {});
  return persistence;
}

async function waitForTextBlockPersistences() {
  await Promise.all([...pendingTextBlockPersistences]);
}
...
fileInput.addEventListener("change", (e) =>
  handleFiles([...e.target.files], { forceNew: true }),
);
...
async function loadAndRenderPdf(file) {
  ...
  try {
    ...
    $("filename").value =
      file.name.replace(/\.pdf$/i, "").trim() || "Sin título";
  }
...
function renderThumbnailsPlaceholder() {
  ...
  for (let i = 0; i < state.totalPages; i++) {
    ...
    cb.addEventListener("change", () => {
      ...
      state.selection = [...$$(".page-thumb-checkbox:checked")].map((el) =>
...
async function renderThumbnailCanvases() {
  ...
  for (const canvasEl of thumbs) {
    ...
    const scale = Math.min(
      (colW * dpr) / baseVp.width,
      (colH * dpr) / baseVp.height,
    );
    ...
    ctx.drawImage(
      tmp,
      Math.round((bufW - tmp.width) / 2),
      Math.round((bufH - tmp.height) / 2),
    );
  }
}
...
async function deleteSelection() {
  ...
  if (
    !confirm(
      `¿Eliminar ${state.selection.length} página${state.selection.length === 1 ? "" : "s"}?`,
    )
  )
    return;
  ...
  try {
    setLoading(true);
    const data = await deletePagesByIndex(state.sessionId, [
      ...state.selection,
    ]);
    state.pages = data.pages;
    state.selection = [];
...
currentPageInput.addEventListener("change", () =>
  navigateTo(Number(currentPageInput.value)),
);
...
async function renderContinuousView() {
  ...
  continuousView
    .querySelectorAll(".continuous-page")
    .forEach((el) => observer.observe(el));
}

btnViewPage.addEventListener("click", () => {
  if (state.viewMode !== "page") setViewMode("page");
});
btnViewCont.addEventListener("click", () => {
  if (state.viewMode !== "continuous") setViewMode("continuous");
});
...
$("btn-confirm-export").addEventListener("click", async () => {
  ...
  try {
    await waitForTextBlockPersistences();
...
function highlightSearchOnPage(pageNum, query) {
  ...
  for (const item of entry.items) {
    ...
    const y =
      canvas.height -
      f * scaleY * state.zoom -
      Math.abs(d) * scaleY * state.zoom;
    const w =
      (item.width ?? item.str.length * Math.abs(a)) * scaleX * state.zoom;
...
compressPanel.addEventListener("click", (e) => {
  if (e.target === compressPanel) closeCompressPanel();
});

$$(".compress-level-btn").forEach((btn) => {
  btn.addEventListener("click", () => {
    $$(".compress-level-btn").forEach((b) =>
      b.classList.remove("compress-level-btn--active"),
    );
  ...
  try {
    ...
    if (reduced) {
      const saving = Math.round(
        (1 - result.sizeBytes / result.originalBytes) * 100,
      );
...
document.addEventListener("keydown", (e) => {
  ...
  if (state.editMode) {
    ...
    if ((e.key === "i" || e.key === "I") && state.sessionId)
...
document.addEventListener("mousemove", (e) => {
  ...
  const w = Math.max(
    160,
    Math.min(maxW, _resizeStartW + e.clientX - _resizeStartX),
  );
...
function createFormFieldElement(field) {
  ...
  if (field.fieldType === "Tx") {
    ...
    input.addEventListener("input", () =>
      onFormValueChange(field.name, input.value),
    );
    ...
    cb.addEventListener("change", () =>
      onFormValueChange(field.name, cb.checked ? "Yes" : "Off"),
    );
    input = cb;
    ...
    radio.checked =
      radio.value !== "" && radio.value !== "Off" && currentVal === radio.value;
    ...
    sel.addEventListener("change", () =>
      onFormValueChange(field.name, sel.value),
    );
    input = sel;
  }
...
function onFormValueChange(name, value) {
  ...
  _saveFormTimeout = setTimeout(async () => {
    ...
    try {
      ...
      /* valores en memoria, sin bloquear */
    }
  }, 600);
}
...
function setActiveTool(tool) {
  ...
  textLayer.classList.toggle(
    "add-text-cursor",
    tool === "addText" || tool === "addRect" || tool === "addCircle",
  );
    ...
    naturalImg.onload = async () => {
      ...
      try {
        ...
        if (state.selectedImageId === tempId)
          state.selectedImageId = imgData.id;
      } catch {
...
function renderEditOverlay() {
  ...
  state.images
    ...
    .forEach((img) => createImageElement(img));
  state.shapes
    .filter((s) => s.pageIndex === pageIdx)
    .forEach((shape) => createShapeElement(shape));
  state.textBlocks
    .filter((b) => b.pageIndex === pageIdx)
    .forEach((block) => createBlockElement(block));

  if (state.formsMode) {
    state.formFields
      .filter((f) => f.pageIndex === pageIdx)
      .forEach((field) => createFormFieldElement(field));
  }
}
...
function createBlockElement(block) {
  ...
  el.addEventListener("mousedown", (e) => {
    ...
    const onUp = () => {
      ...
      if (wasAlreadySelected) enterBlockEditMode(block, content);
    };
...
function deselectShape() {
  ...
  if (!state.selectedBlockId && !state.selectedImageId) {
    ...
    if (!["addRect", "addCircle"].includes(state.activeTool))
...
textLayer.addEventListener("click", async (e) => {
  ...
  const commit = async () => {
    ...
    const text = serializeContentEditableText(content).trim();
    ...
    if (state.sessionId) {
      try {
        const { block: saved } = await trackTextBlockPersistence(
          addTextBlock(state.sessionId, {
...
function enterBlockEditMode(block, content) {
  if (content.contentEditable === "true") return;
  content.contentEditable = "true";
  content.focus();
  ...
  const commit = async () => {
    content.contentEditable = "false";
    const text = serializeContentEditableText(content);
    if (text === block.text) return;
    block.text = text;
    if (state.sessionId) {
      await trackTextBlockPersistence(
        updateTextBlock(state.sessionId, block.id, { text }),
      ).catch(() => {});
    }
  };
  content.addEventListener("blur", commit, { once: true });
...
textLayer.addEventListener("dblclick", (e) => {
  ...
  const block = state.textBlocks.find((b) => b.id === id);
  if (!block) return;
  enterBlockEditMode(block, blockEl.querySelector(".text-block-content"));
});
  ...
  AutoScript.sign(
    ...
    (signatureB64) => {
      ...
      autofirmaBtnDownload.onclick = () => {
        const bytes = Uint8Array.from(atob(signatureB64), (c) =>
          c.charCodeAt(0),
        );
    ...
    (errorType, errorMessage, errorCode) => {
      ...
      const msgs = {
        ...
        SAF_03:
          "Parámetros de firma inválidos. Comprueba que el PDF no está protegido.",
...
signatureCanvas.addEventListener("pointerdown", (e) => {
  ...
  sigCtx.lineWidth =
    e.pressure > 0 && e.pressure < 1 ? Math.max(1, e.pressure * 4) : 2;
