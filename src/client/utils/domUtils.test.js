...
describe("createThumbnailElement", () => {
  it("builds the thumbnail shell with the expected attributes and visible text", () => {
    const thumb = createThumbnailElement(2);

    expect(thumb.className).toBe("page-thumb");
    expect(thumb.dataset.index).toBe("2");
    expect(thumb.querySelector(".page-thumb-handle").title).toBe("Arrastrar");
    expect(thumb.querySelector(".page-thumb-handle").textContent).toBe("⠿");
    expect(
      thumb.querySelector(".page-thumb-canvas-wrapper > canvas").dataset.page,
    ).toBe("3");
    expect(thumb.querySelector(".page-thumb-checkbox").type).toBe("checkbox");
    expect(thumb.querySelector(".page-thumb-checkbox").dataset.index).toBe("2");
    expect(thumb.querySelector(".page-thumb-index").textContent).toBe("3");
  });
});
