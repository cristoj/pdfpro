...
describe("pdfService", () => {
  ...
  test("applyTextBlocks dibuja líneas vacías CRLF con espaciado vertical", async () => {
    const calls = [];
    const doc = {
    ...
    expect(calls).toEqual([
      ["first", expect.objectContaining({ y: 100 })],
      ["", expect.objectContaining({ y: 88 })],
      ["third", expect.objectContaining({ y: 76 })],
    ]);
  });
});
